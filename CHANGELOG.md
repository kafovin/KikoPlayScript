...
tmdb.lua - 0.2.2.220424_build

+ minor feature：微调
  
  + 剧情简介的格式有变化。

    + 资料库的媒体详细信息中，对于标语、本季剧情 (如果有)，剧集/电影介绍、标题，间隔空行显示。例如资料库媒体右键菜单点击`更新`就可以查看。
  ...
  + 设置项有变化
    ...
    + 增加了设置项`搜索 - 关键词作标题`，用来设置搜索时，是否使用关键词直接作为标题，默认不使用。(220423_build)
      此功能可以适用于使用资料库媒体右键菜单点击`搜索媒体信息`获取详细信息时，资料库媒体的标题 与 此脚本当前版本搜索该媒体对应的标题 不符的情况，这样可以维持标题不变。
